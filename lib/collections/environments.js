...
Meteor.methods({
  ...
  environmentImportShared: function(args) {
    const env_values = {
      envName: args.envName
    };
    let envId = Meteor.call('environmentInsert', env_values, function(error, result) {
      ...
      console.log(args);
      args.subjectParameters.envId = new_env._id;
      args.subjectParameters.children.envId = new_env._id;

      Meteor.call('importSubjParameters', args.subjectParameters.children, function(error, result) {
        if (error) {
          throw new Meteor.Error(error.error, error.reason)
        }
        else {
          console.log('sharing students check:', args.shareStudents);
          if (args.shareStudents) {
            console.log('sharing students passed, copying the following students:', args.students);
      ...
      args.sequenceParameters.envId = new_env._id;
      args.sequenceParameters.children.envId = new_env._id;

      Meteor.call('importSeqParameters', args.sequenceParameters.children, function(error, result) {
        if (error) {
          throw new Meteor.Error(error.error, error.reason)
        }
      });
