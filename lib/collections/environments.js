...
class Environment {
  ...
  get students() {
    return Subjects.find({envId: this._id}, {reactive: false}).fetch();
  }

  get studentCount() {
    return Subjects.find({envId: this._id}, {reactive: false}).count();
  }

  participatingStudentCount(obsIds) {
    return this.participatingStudentIds(obsIds).length;
  }
  ...
  participatingStudentPercent(obsIds) {
    return parseFloat(this.participatingStudentCount(obsIds) / this.studentCount).toFixed(4);
  }

  participatingStudentPercentInt(obsIds) {
    return parseFloat(this.participatingStudentPercent(obsIds) * 100).toFixed(2);
  }

  participatingStudentIds(obsIds) {
    return _.uniq(Sequences.find({envId: this._id, obsId: {$in: obsIds}, 'info.student.studentId': {$exists: true, $ne: ''}}, {reactive: false, sort: {'info.student.studentId': 1}, fields: {'info.student.studentId': true}}).fetch().map(seq => seq.info.student.studentId), true);
  }

  participatingStudents(obsIds) {
    return Subjects.find({envId: this._id, _id: {$in: this.participatingStudentIds(obsIds)}}, {reactive: false}).fetch()
  }

  nonParticipatingStudents(obsIds) {
    return Subjects.find({envId: this._id, _id: {$nin: this.participatingStudentIds(obsIds)}}, {reactive: false}).fetch()
  }

  nonParticipatingStudentCount(obsIds) {
    return Subjects.find({envId: this._id, _id: {$nin: this.participatingStudentIds(obsIds)}}, {reactive: false}).count()
  }

  nonParticipatingStudentPercent(obsIds) {
    return parseFloat(this.nonParticipatingStudentCount(obsIds) / this.studentCount).toFixed(4);
  }

  nonParticipatingStudentPercentInt(obsIds) {
    return parseFloat(this.nonParticipatingStudentPercent(obsIds) * 100).toFixed(2);
  }


  get sequences() {
    return Sequences.find({envId: this._id, 'info.student.studentId': {$exists: true, $ne: ''}}, {reactive: false}).fetch();
  }

  get sequenceCount() {
    return Sequences.find({envId: this._id, 'info.student.studentId': {$exists: true, $ne: ''}}, {reactive: false}).count();
  }

  sequenceCountForObservations(obsIds) {
    return Sequences.find({envId: this._id, obsId: {$in: obsIds}, 'info.student.studentId': {$exists: true, $ne: ''}}, {reactive: false}).count();
  }

  get discourseDimensions() {
    return SequenceParameters.findOne({envId: this._id}, {reactive: false});
  }

  get demographics() {
    return SubjectParameters.findOne({envId: this._id}, {reactive: false});
  }

  _studentsInDemographicData(demo, option) {
    let memo_key = `${demo}__${option}`;
    if (!this.memo_data[memo_key]) {
      ...
      this.memo_data[memo_key] = Subjects.find(search, {reactive: false})
    }
    return this.memo_data[memo_key];
  }
  ...
  _sequencesByDemographicData(demo, option, obs_ids) {
    ...
    let search = {
      envId: this._id,
      obsId: {$in: obs_ids},
      'info.student.studentId': {$in: this.studentsInDemographic(demo, option, obs_ids).map(s => s._id)},
    }
    // console.log('search is ', search);
    return Sequences.find(search, {reactive: false})
  }
  ...
  sequencesByDemographicCount(demo, option, obs_ids) {
    return this._sequencesByDemographicData(demo, option, obs_ids).count()
  }
  ...
  _sequencesByDiscourseDimensionData(discdim, option, obs_ids) {
    let search = {
      envId: this._id,
      obsId: {$in: obs_ids},
      'info.student.studentId': {$exists: true, $ne: ''},
    }
    search['info.parameters.' + discdim] = option;

    return Sequences.find(search, {reactive: false})
  }

  sequencesByDiscourseDimension(discdim, option, obs_ids) {
    return this._sequencesByDiscourseDimensionData(discdim, option, obs_ids).fetch()
  }
  sequencesByDiscourseDimensionCount(discdim, option, obs_ids) {
    return this._sequencesByDiscourseDimensionData(discdim, option, obs_ids).count()
  }

  sequencesByDiscourseDimensionPercent(discdim, option, obs_ids) {
    return parseFloat(this.sequencesByDiscourseDimensionCount(discdim, option, obs_ids) / this.sequenceCountForObservations(obs_ids)).toFixed(4)
  }

  sequencesByDiscourseDimensionPercentInt(discdim, option, obs_ids) {
    return parseFloat(this.sequencesByDiscourseDimensionPercent(discdim, option, obs_ids) * 100).toFixed(2);
  }
