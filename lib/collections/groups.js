...
import {envIsSharedToGroup} from "../../helpers/groups";
...
Meteor.methods({
  groupInsert: function(postAttributes) {
    ...
    let params = _.extend(postAttributes, {
      ...
      groupName: postAttributes.groupName,
  ...
  groupEdit: function(options) {
    new SimpleSchema({
      gid: String,
      groupName: String,
    }).validate(options);
    checkAccess(options.gid, 'group', 'admin');

    Groups.update({_id: options.gid}, {$set: {groupName: options.groupName}});
  },
  ...
  groupDelete: function(gid) {
    ...
    checkAccess(gid, 'group', 'admin');

    Groups.remove({_id: gid})
  },
  ...
  addEnvToGroup: function(envid, gid, share_type) {
    ...
    if (envIsSharedToGroup(gid, envid)) {
      throw new Meteor.Error('422', "This environment is already part of the group.")
    }
