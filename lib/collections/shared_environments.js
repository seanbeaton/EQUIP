...
Meteor.methods({
  shareEnvironment: function (envId) {
    ...
    var shareEnv = {
      ...
      sequenceParameters: SequenceParameters.findOne({'envId': envId}),
      subjectParameters: SubjectParameters.findOne({'envId': envId}),
      shareStudents: false,
    };
    ...
    let preexisting_env = SharedEnvironments.findOne({
      shareStudents: false,
      origEnvId: shareEnv.origEnvId,
      envName: shareEnv.envName,
      sequenceParameters: shareEnv.sequenceParameters,
      subjectParameters: shareEnv.subjectParameters
    })
    if (preexisting_env) {
    ...
    let preexisting_stud_share_env = SharedEnvironments.findOne({
      shareStudents: true,
      students: shareEnv.students,
      origEnvId: shareEnv.origEnvId,
      envName: shareEnv.envName,
      sequenceParameters: shareEnv.sequenceParameters,
      subjectParameters: shareEnv.subjectParameters
