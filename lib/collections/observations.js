...
class Observation {
  ...
  get getSequences() {
    return Sequences.find({obsId: this._id, envId: this.envId, 'info.student.studentId': {$exists: true, $ne: ''}}, {reactive: false}).fetch();
  }
  get getSequenceCount() {
    return Sequences.find({obsId: this._id, envId: this.envId, 'info.student.studentId': {$exists: true, $ne: ''}}, {reactive: false}).count();
  }
}

Observations = new Mongo.Collection('observations', {
  transform: (doc) => new Observation(doc)
});
...
Meteor.methods({
  observationInsert: function (observationAttributes) {
    ...
    Meteor.call('environmentModifyObs', observationAttributes.envId);
  ...
  observationRename: function (args) {
    ...
    Meteor.call('environmentModifyObs', args.envId);
