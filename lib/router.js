...
Router.route('/research', {
  name: 'research',
  waitOn: function () {
    return Meteor.subscribe('research_content')
  }
});
...
Router.route('/press', {
  name: 'press',
  waitOn: function () {
    return Meteor.subscribe('press_content')
  }
});
...
Router.route('/reports/v2/interactive', {
  name: 'interactiveReportNew',
});

Router.route('/reports/v2/histogram', {
  name: 'histogramReportNew',
});
...
Router.route('/environment/:_envId', {
  ...
  data: function () {
    return {'envId': this.params._envId}
  },
});
...
Router.route('/environment/:_envId/subjects', {
  name: 'editSubjects',
  data: function () {
    return Environments.findOne(this.params._envId);
  },
  waitOn: function () {
    return [
      Meteor.subscribe('environment', this.params._envId),
...
let filters = {
  admin_only: function () {
    ...
    console.log('admin check user role', Roles.userIsInRole(user, 'admin', 'site'))
...
Router.route('/admin/manage-research', {
  before: filters.admin_only,
  name: 'research_content_manage',
  waitOn: function () {
    return Meteor.subscribe('research_content')
  },
});

Router.route('/admin/manage-press', {
  before: filters.admin_only,
  name: 'press_content_manage',
  waitOn: function () {
    return Meteor.subscribe('press_content')
  },
});
